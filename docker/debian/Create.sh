...
# 启动 Docker 容器
docker run --name debian$ssh_port --device /dev/net/tun --privileged=true -p $ssh_port:22 -p $tran_port_start-$tran_port_end:$tran_port_start-$tran_port_end -d --restart always --cpus 0.$cpu --memory "$ram"m debian$ssh_port /sbin/init
